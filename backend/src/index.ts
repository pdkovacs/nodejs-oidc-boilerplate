...
import { type UserInfo } from "./dto/UserInfo.js";
import { Privilege } from "./security/authorization/privileges/roles-privileges.js";
...
const setupPage = (router: express.Router): void => {
	...
	router.get("/", (req, res) => {
		...
		logger.debug("sending user info: %o...", userInfo);
		res.send({ userInfo });
	});
};
...
const setupAuthRoutes = async (router: express.Router, oidcHandler: OidcHandler, loginUrl: string, logoutUrl: string): Promise<void> => {
	...
	router.post("/logout", (req, res) => {
		...
		try {
			logger.debug("logoutUrl: %s", logoutUrl);
	...
	router.post("/hello", jsonParser, (req, res) => {
		const logger = getLogger("route:///hello");
		logger.debug("req.body: %o", req.body);
		const authn = req.session.authentication;
		if (isEmpty(req.body?.whom)) {
			return res.send({ whom: authn.username });
		} else {
			if (!authn.privileges.includes(Privilege.SAY_HELLO_TO_USER)) {
				return res.sendStatus(403);
			}
			return res.send({ whom: req.body?.whom });
		}
	});
};
